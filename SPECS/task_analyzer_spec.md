...
status = concluida e data_inicio for None;

status = concluida e data_conclusao for None;

data_inicio e data_conclusao estiverem preenchidas e data_conclusao < data_inicio;

qualquer data obrigatória não possuir informação explícita de fuso horário UTC.
...
possuam todos os campos obrigatórios aplicáveis ao seu status;

possuam id_tarefa maior que zero;

possuam prioridade pertencente ao conjunto permitido;

possuam status pertencente ao conjunto permitido;

possuam datas fornecidas em formato válido;

respeitem as regras de consistência de datas definidas neste contrato;

respeitem a regra de unicidade de id_tarefa.
...
possuam status = concluida;

possuam data_inicio válida;

possuam data_conclusao válida;

possuam datas consistentes;

possuam data_conclusao > prazo.
